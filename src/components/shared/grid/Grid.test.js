...
test("Grid component should render items", () => {
    // Arrange
...
test("Grid component should have the right amount of columns", () => {
    // Arrange

    // Act
    const wrapper = shallow(<Grid items={state.items} columns={state.columns} />);

    expect(wrapper.find('th')).toHaveLength(5); //columns are set on api.gnomes.js
});